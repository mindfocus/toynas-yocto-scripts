...
def main(argv):
    ...
    try:
        opts, args = getopt(argv[1:], "he:b:u:t:d:k", ["help", "esr-version=", "bos-version=", "user", "token", "deploy-to", "keep-tmp-dir="])
    ...
    if arg_dict['keepTmpDir'] == True:
        ...
        esr_device_list = create_esr_branches(tmpDir, arg_dict, device_esr_branch)
    else:
    ...
    for device in canonical_device_list:
        if device in esr_device_list:
            if arg_dict['jenkins-user'] != None and arg_dict['jenkins-token'] != None:
                build_and_deploy(device, device_esr_branch, arg_dict['deploy_environment'], arg_dict['jenkins-user'], arg_dict['jenkins-token'])
            continue
        else:
            missing = True
...
def build_and_deploy(device, device_esr_branch, deploy_environment, jenkins_user, jenkins_password):
    # curl -X POST -u ab77:${JENKINS_TOKEN} https://jenkins.dev.resin.io/job/ab77-test/buildWithParameters?foo=bar
    import jenkins
    user = jenkins_user + ":" + jenkins_token
    j = jenkins.Jenkins("https://jenkins.dev.resin.io", username=jenkins_user, password=jenkins_password)
    params = { 'board':device, 'tag':device_esr_branch, 'deployTo':deploy_environment}
    logger.info ("Deploying %s ESR version %s to %s" % (device, device_esr_branch, deploy_environment))
    # Async call - does not report result
...
def create_esr_branches(tmpDir, arg_dict, device_esr_branch):
    esr_device_list = []
    os.chdir(tmpDir)
    ...
    for repourl in repo_list:
        ...
        clone(repourl, arg_dict['bOSVersion'], repoDir)
...
def find_highest_tag(remote, version):
    versions = []
    pat = "v" + version + "*"
    process = Popen(['git', 'ls-remote', '--tags', '--refs',  remote, pat], stdout=PIPE, stderr=PIPE)
    stdout, stderr = process.communicate()
    if stdout:
        prefix = "refs/tags/v"
        ...
        logger.error("No such version in remote repository")
        logger.debug(stderr)
    return None

def checkout_branch_head(remote, version):
    head = find_highest_tag(remote, version)
    if head == None:
               logger.error ("No such release %s in %s" % (version, remote))
               sys.exit(-1)
    repo = git.Repo(os.getcwd())
    repo.git.checkout('-b', head, "refs/tags/v" + head)
    logger.info ("Checked out at refs/tags/v" + head)
    return head
...
def push(remote, branch):
    ...
    try:
        ...
        logger.debug(e)
        if e.status == 1:
            logger.warn("Did not push " + branch + "- already exists")
            return False
        elif e.status == 128:
...
def usage():
    ...
    print ("\t-u Jenkins user name\n")
    print ("\t-t Jenkins API token\n")
    print ("\t-d Balena deploy environment (default to staging\n")
